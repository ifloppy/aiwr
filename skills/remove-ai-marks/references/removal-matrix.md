...
| HTML generator / JSON-LD / embedded data URIs | Strip tags; clean embedded data URIs | `clean_file.py` | Loses meta; cleans embedded rasters; prose under non-naming `<meta>` keys is preserved | Yes |
| Markdown AI frontmatter keys / embedded data URIs | Drop keys; clean embedded data URIs | `clean_file.py` | Loses YAML keys; prose values under non-naming keys are preserved | Yes |
| LaTeX source metadata | Strip `\hypersetup`/`\pdfinfo` provenance fields and tooling comments | `clean_file.py` | Source-level rewrite; aggressive field/comment removal | Yes |
| PDF embedded file attachments | Recursively clean/re-embed with qpdf | `clean_file.py`, `clean_attachments` | PDF rewrite; needs qpdf; nested/oversized failures remain with warnings | Partial |
