...
func TestContainerProseValuesAreNotProvenance(t *testing.T) {
	prose := []string{
		"A generator produced 29 branded PDF datasheets.",
	...
	for _, value := range prose {
		markdown := []byte("---\ntitle: Demo\ncontext: " + value + "\nauthor: you\n---\nBody\n")
		c2pa, ai, findings, _ := inspectMarkdown(markdown)
		if c2pa || ai {
			t.Fatalf("ordinary Markdown prose was flagged: value=%q c2pa=%v ai=%v findings=%v", value, c2pa, ai, findings)
		}
		cleaned, _, err := cleanMarkdown(markdown, DefaultOptions())
		if err != nil || !strings.Contains(string(cleaned), value) || !strings.Contains(string(cleaned), "context:") {
			t.Fatalf("ordinary Markdown prose was damaged: value=%q err=%v cleaned=%q", value, err, cleaned)
		}

		html := []byte(`<html><head><meta name="description" content="` + value + `"></head><body>Body</body></html>`)
		c2pa, ai, findings, _ = inspectHTML(html)
		if c2pa || ai {
			t.Fatalf("ordinary HTML prose was flagged: value=%q c2pa=%v ai=%v findings=%v", value, c2pa, ai, findings)
		}
		cleaned, _, err = cleanHTML(html, DefaultOptions())
		if err != nil || !strings.Contains(string(cleaned), value) {
			t.Fatalf("ordinary HTML prose was damaged: value=%q err=%v cleaned=%q", value, err, cleaned)
		}
	}

	for _, keyValue := range []string{"generator: Claude", "generated_by: Gemini", "tool: OpenAI ChatGPT"} {
		markdown := []byte("---\n" + keyValue + "\ntitle: Demo\n---\nBody\n")
		if _, ai, _, _ := inspectMarkdown(markdown); !ai {
			t.Fatalf("naming key was not flagged: %s", keyValue)
		}
		cleaned, _, err := cleanMarkdown(markdown, DefaultOptions())
		if err != nil || strings.Contains(string(cleaned), strings.Split(keyValue, ":")[0]+":") {
			t.Fatalf("naming key survived clean: %s err=%v cleaned=%q", keyValue, err, cleaned)
		}
	}

	for _, value := range []string{"c2pa manifest attached", "Content-Credentials embedded at export", "SynthID watermark present", "aigc"} {
		markdown := []byte("---\nnote: " + value + "\n---\nBody\n")
		if _, ai, _, _ := inspectMarkdown(markdown); !ai {
			t.Fatalf("unambiguous marker was not flagged: %s", value)
		}
	}
}

func TestHTMLMetadataMarkerOutsideContentIsPreservedAsEvidence(t *testing.T) {
	tag := `<meta property="Claude" content="Claude">`
	if !htmlMetaAI(tag) {
		t.Fatal("marker outside content was hidden by content handling")
	}
	cleaned, _, err := cleanHTML([]byte(`<html><head>`+tag+`</head></html>`), DefaultOptions())
	if err != nil || strings.Contains(string(cleaned), tag) {
		t.Fatalf("AI meta tag survived clean: err=%v cleaned=%q", err, cleaned)
	}

	wordPress := []byte(`<meta name="generator" content="WordPress 6.5">`)
	if _, ai, findings, _ := inspectHTML(wordPress); ai || len(findings) != 1 {
		t.Fatalf("CMS generator was treated as AI: ai=%v findings=%v", ai, findings)
	}
	cleaned, _, err = cleanHTML(wordPress, DefaultOptions())
	if err != nil || !strings.Contains(string(cleaned), "WordPress 6.5") {
		t.Fatalf("CMS generator was damaged: err=%v cleaned=%q", err, cleaned)
	}
}

func TestLatexMetadataAndToolingAreCleaned(t *testing.T) {
	latex := []byte(`% !TEX program = pdflatex
% -*- coding: utf-8 -*-
% Generated by Claude
% vim:ft=tex:sw=2:ts=2
% A normal documentation comment.
...
\end{document}
`)
	if got := classifyBytes(latex, ".tex"); got != KindContainer {
		t.Fatalf("LaTeX classification = %v, want container", got)
	}
	report := inspectContainer(latex, "paper.tex", "latex")
	if !report.HasAIMetadata || report.Details["commands"] != 2 || report.Details["comments_dropped"] != 4 {
		t.Fatalf("LaTeX inspection = %#v", report)
	}
	cleaned, actions, err := cleanContainer(latex, "latex", DefaultOptions())
	if err != nil {
		t.Fatal(err)
	}
	got := string(cleaned)
	for _, removed := range []string{"pdfauthor", "pdfcreator", "pdfproducer", "/Author", "/Creator", "/Producer", "% !TEX", "% -*-", "% Generated by Claude", "% vim:"} {
		if strings.Contains(got, removed) {
			t.Fatalf("LaTeX provenance survived: %q in %q", removed, got)
		}
	}
	for _, kept := range []string{"colorlinks=true", "pdftitle={My Paper}", `\author{Jane Doe}`, `\title{My Paper}`, `\textbf{world}`, "% A normal documentation comment."} {
		if !strings.Contains(got, kept) {
			t.Fatalf("LaTeX document content was damaged: missing %q in %q", kept, got)
		}
	}
	if !containsString(actions, "drop comment: AI markers") || !containsString(actions, "drop hypersetup pdfcreator") {
		t.Fatalf("LaTeX cleanup actions = %#v", actions)
	}
}

func TestLatexLiteralExamplesArePreserved(t *testing.T) {
	latex := []byte(`\documentclass{article}
\begin{verbatim}
...
`)
	cleaned, _, err := cleanContainer(latex, "latex", DefaultOptions())
	if err != nil || !strings.Contains(string(cleaned), `\hypersetup{pdfcreator={Claude}}`) || !strings.Contains(string(cleaned), "% example config") {
		t.Fatalf("literal LaTeX example was damaged: err=%v cleaned=%q", err, cleaned)
	}
	if _, ai, findings, _ := inspectLaTeX(latex); ai || len(findings) != 0 {
		t.Fatalf("literal LaTeX example was flagged: ai=%v findings=%v", ai, findings)
	}
}
